...
(function () {
  ...
  const renderer = new THREE.WebGLRenderer({ canvas: threeCanvas, antialias: false });
  ...
  /* ── 2-D neural rain ─────────────────────────────────────── */
  const COL_W = innerWidth > 1200 ? 34 : 28; // Less dense on large screens
  const NUM_COL = Math.min(Math.ceil(innerWidth / COL_W) + 2, 80); // Cap max columns
  ...
  const rainCols = Array.from({ length: NUM_COL }, (_, ci) => ({
    ...
    speed: rand(1.2, 3.5), // Slightly slower base speed
    len: Math.floor(rand(4, 14)), // Shorter tails for performance
  ...
  function tickRain(dt, alpha) {
    ...
    rainCols.forEach(col => {
      ...
      for (let k = 0; k < col.len; k++) {
        ...
        // Skip rendering dim tail characters if alpha is very low
        if (alpha < 0.2 && k > 4) continue;
